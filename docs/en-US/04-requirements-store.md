...
```
Seller applies → verify user exists and has SELLER role
...
**Precondition**: Logged in with SELLER role.

**Rules**: Max 1 active store per user (checked via SELECT COUNT). Initial status PENDING(0). Store name required.

**Errors**: USER_NOT_FOUND(201004) / NOT_SELLER_ROLE(202005) / STORE_LIMIT_EXCEEDED(202001)
